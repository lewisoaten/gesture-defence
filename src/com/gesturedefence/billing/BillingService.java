...
public class BillingService extends Service implements ServiceConnection{
	...
	// ========================================
		...
		public BillingService(GestureDefence baseThing)
		{
			super();
			this.base = baseThing;
		}
		...
		private void runPendingRequests() {
			...
			while ((request = mPendingRequests.peek()) != null)
			{
				if (request.runIfConnected())
				{
					...
					/* Remember the largest startId, which is the most recent request to start this service.*/
					if (maxStartId < request.getStartId())
						maxStartId = request.getStartId();
				} else
				{ 
					/* The service crashed, so restart it. Note that thus leaves the current request on the queue.*/
	...
	// ========================================
		 ...
		 * If we are already connected to the MarketBillingService, then we call the run() method directly.
		...
		public class RequestPurchase extends BillingRequest {
			...
			@Override
			protected void responseCodeRecieved(ResponseCode responseCode) {
				ResponseHandler.responseCodeRecieved(BillingService.this, this, responseCode);
			}
		}
